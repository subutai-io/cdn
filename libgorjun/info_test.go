...
*/	/*	str, err := v.Upload(fmt.Sprintf("data/%s/mysql-subutai-template_0.3.9_amd64.tar.gz", v.Username), "template", "false")
	...
	str, err = v.Upload(fmt.Sprintf("data/%s/mysql-subutai-template_0.4.0_amd64.tar.gz", v.Username), "template", "false")
	if err != nil {
		fmt.Printf("Could not upload mysql-subutai-template_0.4.0_amd64.tar.gz: %+v, %+v\n", str, err)
	}
	str, err = v.Upload(fmt.Sprintf("data/%s/mysql-subutai-template_0.4.1_amd64.tar.gz", v.Username), "raw", "false")
	if err != nil {
...
*/	f := FirstGorjunUser()
	...
	f.AuthenticateUser()
	fmt.Printf("Token for user %+v = %+v\n", f.Username, f.Token)
	str, err := f.Upload(fmt.Sprintf("data/akenzhaliev/debian-stretch-subutai-template_0.4.1_amd64.tar.gz"), "raw", "false")
	if err != nil {
		fmt.Printf("Could not upload debian-stretch-subutai-template_0.4.1_amd64.tar.gz: %+v, %+v\n", str, err)
	}
/*	str, err = f.Upload(fmt.Sprintf("data/%s/debian-stretch-subutai-template_0.4.1_amd64-%s.tar.gz", f.Username, f.Username), "raw", "false")
	...
	str, err = f.Upload(fmt.Sprintf("data/%s/debian-stretch-subutai-template_0.4.1_amd64-%s.tar.gz", f.Username, f.Username), "template", "false")
	if err != nil {
		fmt.Printf("Could not upload debian-stretch-subutai-template_0.4.1_amd64-%s.tar.gz: %+v, %+v\n", f.Username, str, err)
	}
	str, err = f.Upload(fmt.Sprintf("data/%s/mysql-subutai-template_0.4.1_amd64-%s.tar.gz", f.Username, f.Username), "template", "false")
	if err != nil {
		fmt.Printf("Could not upload mysql-subutai-template_0.4.1_amd64-%s.tar.gz: %+v, %+v\n", f.Username, str, err)
	}
*//*	str, err := f.Upload(fmt.Sprintf("data/%s/mysql-subutai-template_0.4.1_amd64.tar.gz", v.Username), "template", "true")
	if err != nil {
		fmt.Printf("Could not upload mysql-subutai-template_0.4.1_amd64.tar.gz: %+v, %+v\n", str, err)
	}*/
/*	s := SecondGorjunUser()
	s.Register(s.Username)
	s.AuthenticateUser()
	fmt.Printf("Token for user %+v = %+v\n", s.Username, s.Token)
*//*	str, err = s.Upload(fmt.Sprintf("data/%s/debian-stretch-subutai-template_0.4.1_amd64-%s.tar.gz", s.Username, s.Username), "template", "false")
	if err != nil {
		fmt.Printf("Could not upload debian-stretch-subutai-template_0.4.1_amd64-%s.tar.gz: %+v, %+v\n", s.Username, str, err)
	}
	str, err = s.Upload(fmt.Sprintf("data/%s/mysql-subutai-template_0.4.1_amd64-%s.tar.gz", s.Username, s.Username), "template", "false")
	if err != nil {
		fmt.Printf("Could not upload mysql-subutai-template_0.4.1_amd64-%s.tar.gz: %+v, %+v\n", s.Username, str, err)
	}
