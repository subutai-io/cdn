...
names=( "vagrant-subutai-stretch-virtualbox.box" "vagrant-subutai-stretch-libvirt.box" "vagrant-subutai-stretch-hyperv.box" "vagrant-subutai-stretch-parallels.box" "vagrant-subutai-stretch-vmware.box" "management" "generic-ansible" "debian-stretch" "XQuartz.pkg" "googlechrome.dmg" "Firefox-61.0.1.dmg" "vmware-fusion-osx.dmg" "VirtualBox.pkg" "vmware-workstation-windows.exe" "vagrant.msi" "VirtualBox.exe" "vmware-workstation-linux.bundle" "vagrant.deb" "virtualbox-xenial.deb" "virtualbox-stretch.deb" )

n=0

while [ $n -lt ${#objects[@]} ] ; do
    DISPLAY_NAME[$n]=${objects[$n]}" "${names[$n]}
    for object in ${objects[$n]} 
        do 
            echo "Pinning ${DISPLAY_NAME[$n]}"
